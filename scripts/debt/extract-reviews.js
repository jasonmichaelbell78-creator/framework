...
// eslint-disable-next-line complexity -- processReviewItem has inherent branching (complexity 25), refactoring would reduce readability
...
// eslint-disable-next-line complexity -- main has inherent branching (complexity 25), refactoring would reduce readability
async function main() {
  ...
  for (const file of reviewFiles) {
    ...
    try {
      ...
      console.warn(`  Failed to read ${relPath}: ${errMsg}`);
      continue;
    }
    ...
    for (const line of lines) {
      try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err); // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
        console.warn(`  Failed to parse line in ${relPath}: ${errMsg}`);
      }
    }
