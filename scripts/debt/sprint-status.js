...
function formatPipelineSection(p) {
  return [
    '--- Pipeline Health ---',
    `  ${p.dedupedSynced ? '[OK]' : '[!!]'} Deduped synced: ${p.dedupedSynced}`,
    `  ${p.metricsStale ? '[!!]' : '[OK]'} Metrics freshness: ${p.metricsAge}${p.metricsStale ? ' (STALE)' : ''}`,
    `  ${p.roadmapS0Stale ? '[!!]' : '[OK]'} ROADMAP S0: ${p.roadmapS0Shown} shown / ${p.roadmapS0Actual} actual${p.roadmapS0Stale ? ' (STALE)' : ''}`,
    `  ${p.viewsStale ? '[!!]' : '[OK]'} Views freshness: ${p.viewsStale ? 'STALE' : 'OK'}`,
    '',
  ];
}
...
const STATUS_LABELS = { COMPLETE: '[DONE]', ACTIVE: '[ACTIVE]' };

function formatAllSprintsSection(allSprints) {
  const lines = ['', '--- All Sprints ---'];
  for (const s of allSprints) {
    const statusLabel = STATUS_LABELS[s.status] || '[PLANNED]';
    lines.push(
      `  ${statusLabel} ${s.id.padEnd(12)} ${s.status.padEnd(10)} ${s.resolved}/${s.total} resolved (${s.remaining} remaining)`,
    );
  }
  lines.push('');
  return lines;
