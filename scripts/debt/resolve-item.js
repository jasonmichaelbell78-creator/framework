...
function findItem(items, debtId) {
  ...
  if (item.status === 'RESOLVED') {
    console.log(`Item ${debtId} is already RESOLVED`);
    console.log(`  Resolved on: ${item.resolution?.date || 'unknown'}`);
    process.exit(0);
  }
  if (item.status === 'FALSE_POSITIVE') {
    console.log(`Item ${debtId} is already marked as FALSE_POSITIVE`);
    process.exit(0);
  }
...
function applyFalsePositive(items, item, itemIndex, parsed, now, masterBackup) {
  ...
  // Sync deduped.jsonl to prevent generate-views.js overwrite regression
  try {
    const { execFileSync } = require('node:child_process');
