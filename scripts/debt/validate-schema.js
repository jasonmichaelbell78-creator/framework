...
// eslint-disable-next-line complexity -- validateItem has inherent branching (complexity 48), refactoring would reduce readability
...
// Main function
// eslint-disable-next-line complexity -- main has inherent branching (complexity 39), refactoring would reduce readability
async function main() {
...
`);
  ...
  try {
    ...
    console.error(
      `Error reading file: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(2);
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      allErrors.push(
        `Line ${lineNum}: JSON parse error: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
      );
    }
  }
