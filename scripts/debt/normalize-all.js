...
// eslint-disable-next-line complexity -- normalizeItem has inherent branching (complexity 24), refactoring would reduce readability
...
async function main() {
  ...
  if (files.length === 0) {
    console.log('No raw extraction files found. Run extract scripts first.');
    process.exit(1);
  }
  ...
  for (const file of files) {
      ...
      const msg =
        error_ instanceof Error
          ? error_ instanceof Error
            ...
            : String(error_)
          : String(error_);
      console.warn(`  Failed to read ${path.basename(file)}: ${msg}`);
      continue;
    }
    ...
    for (const line of lines) {
      try {
        ...
        console.warn(
          `  Failed to parse line in ${fileName}: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
        );
      }
    }
