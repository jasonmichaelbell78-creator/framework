...
function carryForward(sprintId, carryTo, remaining, debtMap, manifest) {
  ...
  try {
    ...
    console.error(
      `Error: Failed to write carry-to manifest: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(2);
  }
...
function main() {
  ...
  try {
    ...
    console.error(
      `Error: Failed to write grand-plan-manifest.json: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(
      `Error: Failed to write completion log: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(
      `Error: Failed to write sprint report: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(2);
  }
