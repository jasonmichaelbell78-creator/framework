...
// Main function
// eslint-disable-next-line complexity -- main has inherent branching (complexity 45), refactoring would reduce readability
async function main() {
...
`);
  ...
  try {
    ...
    console.warn('  reconcile-roadmap.js failed -- run manually if needed');
  }
  try {
    ...
    console.warn('  generate-grand-plan.js failed -- run manually if needed');
  }
  ...
  if (refsFound.length > 0) {
    console.log(`\n  Resolved items also referenced in:`);
