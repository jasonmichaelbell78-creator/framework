...
const MASTER = path.join(DEBT_DIR, 'MASTER_DEBT.jsonl');
const SPRINT_DIR = path.join(DEBT_DIR, 'logs');
const ROADMAP = path.join(ROOT, 'ROADMAP.md');
const GRAND_PLAN = path.join(DEBT_DIR, 'GRAND_PLAN_V2.md');
const INDEX = path.join(DEBT_DIR, 'INDEX.md');
...
const coveragePct = hasOpen
  ? // eslint-disable-next-line framework/no-unsafe-division -- openItems.length is checked for > 0 in surrounding context
...
if (Array.isArray(unplaced) && unplaced.length > 0) {
  console.log('');
  console.log('--- UNPLACED ITEMS (' + (Array.isArray(unplaced) ? unplaced.length : 0) + ') ---');
