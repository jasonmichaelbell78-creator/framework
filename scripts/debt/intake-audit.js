...
const _VALID_STATUSES = schema.validStatuses;
...
function loadMasterDebt() {
  ...
  try {
    ...
    console.error(
      ...
      `Warning: Failed to read MASTER_DEBT.jsonl: ${error_ instanceof Error ? error_.message : String(error_)}`,
    );
    return [];
  }
  ...
  if (badLines.length > 0) {
    console.error(`Warning: ${badLines.length} invalid JSON line(s) in MASTER_DEBT.jsonl`);
...
function _findDuplicate(item, existingItems) {
...
function printFilePathWarnings(filePathWarnings) {
  ...
  console.log(`    TDMS requires real file paths. Fix before committing.`);
}
...
// eslint-disable-next-line complexity -- printIntakeReport has inherent branching (complexity 19), refactoring would reduce readability
...
// Main function
// eslint-disable-next-line complexity -- main has inherent branching (complexity 41), refactoring would reduce readability
async function main() {
  ...
  for (let i = 0; i < inputLines.length; i++) {
    ...
    try {
      ...
      errors.push({
        ...
        errors: [
          `JSON parse error: ${err instanceof Error ? err.message : String(err)}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
        ],
      });
    }
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err); // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    console.warn(`  Multi-pass dedup failed: ${msg}. Falling back to views-only regeneration.`);
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err); // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    console.warn(
      `  Failed to regenerate views: ${msg}. Run manually: node scripts/debt/generate-views.js`,
    );
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err); // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    console.warn(
      `  Failed to assign roadmap refs: ${msg}. Run manually: node scripts/debt/assign-roadmap-refs.js`,
    );
  }
