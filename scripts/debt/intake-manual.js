...
function loadMasterDebt() {
  ...
  for (let i = 0; i < lines.length; i++) {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : String(err); // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
...
// eslint-disable-next-line complexity -- validateInput has inherent branching (complexity 19), refactoring would reduce readability
...
`);
  ...
  if (existingHashes.has(newItem.content_hash)) {
    ...
    console.log(`Duplicate detected! This item already exists as ${duplicate?.id || 'unknown'}`);
