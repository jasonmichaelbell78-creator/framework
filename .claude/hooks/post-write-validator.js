 ...
 *
 * Runs validators on written/edited files and outputs structured results.
 * Validators are organized in three tiers:
 *   - BLOCKING: fails the operation if triggered
 *   - WARN: emits warnings to stderr, does not block
 *   - SUGGEST: emits suggestions to stderr, does not block
 *
 * Validators included:
 ...
 *   4. componentSizeCheck    — component size limit for TSX files (WARN)
 *   5. typescriptStrictCheck — `any` type usage (WARN)
 *   6. testMockingValidator  — bad test mocking detection (BLOCK)
 *   7. agentTriggerEnforcer  — track modifications, suggest agents (SUGGEST)
 *
...
const _isJsTsFile = /\.(ts|tsx|js|jsx)$/.test(filePath);
...
// ─── Validator: testMockingValidator (BLOCK) ──────────────────────────────────
// Blocks mocking of core infrastructure modules in tests — keeps tests honest.
...
function testMockingValidator() {
  ...
  // Patterns that indicate mocking core modules instead of the proper abstraction layer
  // Projects should customize this list for their own infrastructure modules.
  const BAD_MOCK_PATTERNS = [
    // Example: block direct mocking of database modules
    // /vi\.mock\s*\(\s*["']your-db-module["']/,
    // /jest\.mock\s*\(\s*["']your-db-module["']/,
  ];

  if (BAD_MOCK_PATTERNS.length === 0) return;
  ...
  if (found) {
    ...
    console.error('');
    console.error('Tests must mock at the abstraction layer, not core infrastructure directly.');
    console.error('This ensures tests validate security and integration layers.');
    console.error('\u2501'.repeat(28));
    blockReason = 'Tests must mock at the abstraction layer, not core infrastructure directly';
  }
}

// ─── Validator: auditS0S1 (WARN/BLOCK, Write only) ────────────────────────────

// eslint-disable-next-line complexity -- auditS0S1 has inherent branching (complexity 38), refactoring would reduce readability
function auditS0S1() {
...
function patternCheck() {
  ...
  if (violations.length > 0) {
    ...
    if (violations.length > 5) console.error(`   ... and ${violations.length - 5} more`);
    console.error('');
    console.error('Review docs/CODE_PATTERNS.md for documented patterns.');
    console.error('\u2501'.repeat(28));
  }
}

// ─── Validator: componentSizeCheck (WARN) ────────────────────────────────────
...
// ─── Validator: typescriptStrictCheck (WARN) ─────────────────────────────────
...
// ─── Validator: checkRequirements (SUGGEST) ───────────────────────────────────

// eslint-disable-next-line complexity -- checkRequirements has inherent branching (complexity 17), refactoring would reduce readability
...
// ─── Validator: agentTriggerEnforcer (SUGGEST) ────────────────────────────────

// eslint-disable-next-line complexity -- agentTriggerEnforcer has inherent branching (complexity 43), refactoring would reduce readability
function agentTriggerEnforcer() {
  ...
  // Prune suggestedAgents entries older than 30 days
  const PRUNE_DAYS = 30;
