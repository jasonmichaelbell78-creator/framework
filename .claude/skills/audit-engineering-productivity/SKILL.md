...
- `src/**/*.ts` (backend/service logging)
- `src/**/error.tsx` (error boundaries)
...
- Offline persistence configuration (IndexedDB, localStorage, cache API)
...
- `lib/persistence.ts` or equivalent (if exists)
- `public/sw.js` (if exists)
- `components/status/offline-indicator.tsx` (if exists)
...
1. Check offline persistence configuration
...
- `/audit-comprehensive` - Run all domain audits
