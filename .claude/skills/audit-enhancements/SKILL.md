...
description: Run a comprehensive enhancement audit across the entire project - code,
...
grep -E '"(react|typescript)"' package.json | head -5
```
...
**Files**: `src/`, `components/`, `lib/`, `hooks/`, `types/` **Categories**:
`app-architecture`
...
**Files**: `src/`, `components/`, `styles/`, `public/` **Categories**:
...
- Sensitive content areas: flag findings that touch sensitive areas but do NOT
  deep-dive into domain-specific content itself

### Agent 3: Content Scanner

**Focus**: Microcopy, button labels, error messages, onboarding text, help text
**Files**: `src/`, `components/` (string literals, i18n files) **Categories**:
`content`
...
**Focus**: Build config, dependency choices, deployment
**Files**: `package.json`, framework config files, deployment configs
...
- Configuration improvements (security, caching headers)
- Framework config optimization
...
**Files**: `docs/`, `*.md`, `CLAUDE.md` **Categories**:
...
- External service optimization (monitoring, analytics)
...
Use these examples to calibrate severity ratings. Agents tend to over-rate
severity — use these as anchors:

| Severity | Definition                  | Example                                                     |
| -------- | --------------------------- | ----------------------------------------------------------- |
| S0       | Transformative / structural | Migrate to different database; rewrite auth system          |
